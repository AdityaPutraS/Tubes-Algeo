public class MatrixInterpolasi extends Matrix {
    ...
    public void titikMatrix(float[][] matrixTitik){
        int i,j;
        for (i=0;i<this.nBrs;i++){
            for(j=0;j<this.nKol-1;j++){
                this.data[i][j]=(float)Math.pow((double)matrixTitik[i][0],(double)this.nKol-j-2);
            }
    ...
    public void printHasilInterpolasi(float x){
        float sum=0;
        for(int i=0;i<this.getnBrs();i++){
            sum+=this.data[i][this.getnKol()-1]*(float)Math.pow((double)x,(double)(this.getnBrs()-1-i));
        }
